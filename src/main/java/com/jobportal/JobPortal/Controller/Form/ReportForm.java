...
public class ReportForm {
    ...
    //説明会.
    @NotBlank(message = "必須項目です", groups = briefingGroup.class)
    private String briefingContent;
    @NotBlank(message = "必須項目です", groups = briefingGroup.class)
    private String briefingImpressions;

    //試験.
    @Max(value = 100, groups = testGroup.class)
    @Min(value = 0, groups = testGroup.class)
    private Integer generalKnowledgeNumber;//一般常識問題数
    @Size(max = 64, message = "64文字以内で入力してください",groups = testGroup.class)
    ...
    @NotEmpty(message = "必須項目です", groups = testGroup.class)
    private String testImpressions;//感想

    //面接.
    @NotNull(message = "必須項目です", groups = interviewGroup.class)
    private Integer interviewerNumber;
    @NotBlank(message = "必須項目です", groups = interviewGroup.class)
    ...
    //研修.
    @NotBlank(message = "必須項目です", groups = trainingGroup.class)
    private String trainingImpressions;
    //内定式.
    @NotBlank(message = "必須項目です", groups = informalCeremonyGroup.class)
    private String informalCeremonyImpressions;
    //セミナー.
    @NotEmpty(message = "最低一つは入力してください", groups = reportSeminarGroup.class)
    @Valid
    private List<ReportSeminarForm> seminarForms;

    //その他.
    @NotBlank(message = "必須項目です", groups = reportOtherGroup.class)
    private String activityContent;
    @NotBlank(message = "必須項目です", groups = reportOtherGroup.class)
    private String otherImpressions;

    public ReportForm(ReportInfoEntity mainInfo, ReportBriefingEntity briefingEntity){
        this.reportId = mainInfo.getReportId();
        this.reason = String.valueOf(mainInfo.getReason());
        this.activityTime = mainInfo.getActivityTime();
        this.companyName = mainInfo.getCompanyName();
        this.employmentIntention = String.valueOf(mainInfo.getEmploymentIntention());
        this.nextAction = String.valueOf(mainInfo.getNextAction());
    ...
    public ReportForm(ReportInfoEntity mainInfo, ReportOtherEntity otherEntity){
        ...
        this.otherImpressions = otherEntity.otherImpressions();
    }
